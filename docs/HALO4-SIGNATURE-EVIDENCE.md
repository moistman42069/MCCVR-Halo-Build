...
### E-H4-21: first-person animation producer and final orientation-bank write (PROVEN 2026-08-08)

The user's C-H4-11b result was decisive: the node bank was readable, had the
proven `{quaternion, translation, scale}` shape, accepted a guarded write and
immediately read it back, but the gun remained attached to the face. That
establishes only that C-H4-11b wrote before a later engine producer; it does
not establish that the orientation bank is absent from the final draw.

H4EK `halo4_tag_test.exe+0x92B470` is that producer. Its measured ABI is
`(ecx=output_user_index, edx=weapon_slot, r8b=enabled)`. It obtains the
first-person weapon data for that user and slot, derives the orientation record
with the kit's `user * 0x5F48 + slot * 0x1E00 + 0x678` path, then initializes
and fills individual 0x20-byte node transforms at `orientation + node * 0x20`.
The default write is at kit `0x92B76E`; mapped animation transforms are copied
at `0x92B841` and `0x92B931`. Those are the same node transforms C-H4-11b
observed, but rebuilt after its wrapper-time write.

The pinned retail Steam `halo4.dll` homolog is function entry
`0x3B6D04`. Its body independently repeats the TLS `0x6A0`, user `0x5F48`,
weapon `0x2EC8`, orientation `0x1E00` and node `<< 5` calculations, and writes
the node records at `0x3B6F65` and `0x3B6FFF`. The following 65-byte pattern
matches the pinned image **exactly once** at `0x3B6D7F`, precisely `0x7B` bytes
after that entry:
...
**C-H4-12 scope.** Hook this producer only after the mandatory camera core is
already armed. The detour calls Halo 4 first, then transforms only the
producer's declared user/weapon orientation bank with the controller pose.
This orders the existing safe node-format probe after the final known writer.
The signature is optional: zero, multiple, moved, or MinHook-rejected matches
leave hands stock and do not alter camera ownership, stereo, or session state.
The remaining headset test is behavioral: moving the controller must move the
gun and visible arms relative to the head rather than leave them face-locked.
